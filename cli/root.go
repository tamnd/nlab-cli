// Package cli builds the nlab command tree on top of the nlab library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: nlab.DefaultConfig()}

	root := &cobra.Command{
		Use:   "nlab",
		Short: "Browse the nLab mathematics wiki",
		Long: `nlab reads the nLab mathematics wiki through the public MediaWiki JSON API.
No API key is required. It returns records as table, JSON, JSONL, CSV, TSV, or URLs.

nlab is an independent tool and is not affiliated with the nLab or its contributors.`,
		SilenceUsage:  true,
		SilenceErrors: true,
