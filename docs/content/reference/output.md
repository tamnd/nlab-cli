...
describes what users get. Pick a format with `-o`, or let nlab choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
nlab <command> -o table   # aligned columns for reading
nlab <command> -o jsonl   # one JSON object per line, for piping
nlab <command> -o json    # a single JSON array
nlab <command> -o csv     # spreadsheet friendly
nlab <command> -o tsv     # tab-separated
nlab <command> -o url     # just the URL column
nlab <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
nlab <command> --fields id,title,url
```
...
```bash
nlab <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
nlab <command>            # a table, because this is a terminal
nlab <command> | wc -l    # JSONL, because this is a pipe
```
